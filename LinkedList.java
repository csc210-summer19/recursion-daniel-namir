...
public class LinkedList<E extends Comparable<E>> {
	...
	// This function is a recursive function that removes all occurrences of the
	// given element in the LinkedList, and decrements the size of the list (n).
	private void removeAll(Node ref, E el, Node previous) {
		if (ref == null)
			return;
		else {
			if (ref.data.equals(el)) {
				...
				n--;
				removeAll(ref.next, el, previous);
			} else
				removeAll(ref.next, el, ref);
		}
	}
	...
	// This function is a recursive function that duplicates the given element in
	// the LinkedList in all its occurrences, and increases the size of the list.
	private void duplicateAll(E el, Node ref) {
		if (ref == null)
			return;
		if (ref.data.equals(el)) {
			Node temp = new Node(el);
			Node holder = ref.next;
			ref.next = temp;
			temp.next = holder;
			ref = ref.next.next;
			n++;
